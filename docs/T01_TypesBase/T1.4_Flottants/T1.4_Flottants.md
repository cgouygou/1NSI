...
Avant de commencer, demandez à Python d'effectuer les calculs suivants (que vous aurez bien entendu faits de tête) dans un terminal:
```python
>>> 0.1 + 0.2
...
!!! abstract "Base 2 → Base 10"
    ...
    ** Remarque** si l'écriture en base 2 est finie, l'écriture en base 10 est également finie.
...
Un nombre admettant une incriture binaire **infinie** ne peut pas être correctement représenté: le nombre manipulé sur la machine est donc **une valeur approchée**.  Il faut garder en tête que les calculs sont potentiellement faux, du moins imprécis, lorsque des flottants interviennent.
...
!!! history "Histoire de l'informatique: Antimissile Patriot"
    ...
    Ainsi un *Patriot* est passé à plus de 500 mètres d'un *Scud* le 25 février 1991 qui a causé la mort de 28 personnes sur la caserne de Dahran.
