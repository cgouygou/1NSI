...
## 3. Compléments sur les commandes
...
!!! example "TP noté"
    === "Énoncé"
        ...
        9. On peut écrire du texte (par exemple «Bonjour») dans un fichier (par exemple «toto.txt») grâce à la commande `echo Bonjour > toto.txt`. Pour ajouter du texte, on remplace `>` par `>>`. Créez un fichier `reponses.txt` dans lequel vous écrirez les réponses aux questions 6, 7 et 8.
        ...
        Vous pouvez maintenant aller sur Moodle. Dans le dépôt «TP shell» de la section «Évaluations», déposez le fichier `.bash_history` situé dans `/home/nsi/`. Si vous ne le voyez pas, affichez les fichiers cachés.
        "
        ) }}
    === "Correction" 
        {{ correction(False, 
