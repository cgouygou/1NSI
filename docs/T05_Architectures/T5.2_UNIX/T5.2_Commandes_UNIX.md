...
!!! abstract "Repertoire courant et chemins"
    ...
    === "Répertoire courant"
        ...
        Par exemple, si Elsa décide de suivre le cours sur les tables dans un fichier `tables.py` qu'elle enregistrera dans le répertoire `boulot` avec Thonny, le répertoire courant pour Thonny sera `boulot`. Il est alors impératif d'enregistrer le fichier `sw.csv` dans **ce répertoire** pour pouvoir l'importer avec `#!py open('sw.csv')`. 
    ...
    === "Chemin relatif"
        ...
        Si par mégarde Elsa a enregistré le fichier `sw.csv` dans le répertoire `elsa`, il faudra l'importer avec `#!py open('../../sw.csv')` .
...
!!! example "{{ exercice() }}"
    === "Énoncé" 
        ...
        6. Consulter le manuel de la commande `echo`.
        ...
        9. Ajouter cette réponse au fichier `reponses.txt`.
    === "Correction" 
        {{ correction(False, 
        "
        "
