...
!!! example "Partie 3: Ajout d'un serveur DNS"
    ...
    4. Sur ce serveur DNS, associer l'adresse ```http://www.vivelansi.fr```  à l'adresse IP ```192.168.1.30```.
...
!!! example "{{ exercice() }}"
    === "Énoncé" 
        On souhaite pouvoir raccorder 1000 machines sur le même réseau IP. Donner le plus petit masque permetteant de définir un tel sous-réseau. Même question avec 3000 machines.
