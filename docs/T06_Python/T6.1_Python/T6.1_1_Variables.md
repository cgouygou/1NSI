...
{{ initexo(0) }}
!!! example "{{ exercice() }}: Affichage"
    === "Énoncé" 
        Lire le code ci-dessous et **anticiper** la valeur contenue dans chaque variable, puis copier-coller le code et l'exécuter pour vérifier.
        ...
        {{ IDEv('foo/bar/ex1', MAX = 1000) }}

        !!! info "Remarque"
            vous l'aurez compris, la fonction `print` sert à afficher une ou plusieurs variables (ou tout simplement une valeur)... 
...
!!! example "{{ exercice() }}: Incrémentation"
    === "Énoncé" 
        ...
        !!! info "Remarque"
            Ajouter 1 (ou un autre valeur entière fixée) à une variable, de façon répétitive, s'appelle **incrémenter** la variable.
