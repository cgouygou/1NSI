...
!!! abstract "Définir une fonction - syntaxe générale"
    Pour définir une fonction, on utilise le mot-clé `def` suivi du nom de la fonction, avec le(s) paramètre(s) entre parenthèses. Le bloc d'instructions consituant **le corps** de la fonction doit être indenté.
